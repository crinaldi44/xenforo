<?php
// FROM HASH: 394a45e385302380d65a445fd8211204
return array(
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['includeMicrodata']) {
		$__finalCompiled .= '
			<meta itemprop="parentItem" itemscope itemid="' . $__templater->func('link', array('canonical:threads', $__vars['thread'], ), true) . '" />
		';
	}
	$__finalCompiled .= '
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	$__finalCompiled .= '
			<li class="u-concealed">
				<a href="' . $__templater->func('link', array('threads/post', $__vars['thread'], array('post_id' => $__vars['post']['post_id'], ), ), true) . '" rel="nofollow" ' . ($__vars['includeMicrodata'] ? 'itemprop="url"' : '') . '>
					' . $__templater->func('date_dynamic', array($__vars['post']['post_date'], array(
		'itemprop' => ($__vars['includeMicrodata'] ? 'datePublished' : ''),
	))) . '
				</a>
			</li>
			';
	...
	if ($__vars['xf']['options']['embedCodeShare'] AND $__templater->func('method_exists', array($__vars['post'], 'getEmbedCodeHtml', ), false)) {
		$__finalCompiled .= '
			...
			'text' => $__templater->method($__vars['post'], 'getEmbedCodeHtml', array()),
	...
	$__compilerTemp1 = '';
	$__compilerTemp1 .= '
						' . $__templater->callMacro(null, 'bookmark_macros::link', array(
		'content' => $__vars['post'],
...
'code' => function($__templater, array $__vars, $__extensions = null)
{
	...
	if ($__vars['post']['message_state'] == 'deleted') {
		...
		<div class="messageNotice messageNotice--deleted">
			' . $__templater->callMacro(null, 'deletion_macros::notice', array(
	...
	if ($__vars['post']['last_edit_date']) {
		...
		if ($__vars['post']['user_id'] == $__vars['post']['last_edit_user_id']) {
			$__finalCompiled .= '
				...
				'itemprop' => ($__vars['includeMicrodata'] ? 'dateModified' : ''),
			))) . '
			';
		} else {
			$__finalCompiled .= '
	...
	if ($__vars['includeMicrodata']) {
		$__finalCompiled .= '
			<div class="message-microdata" itemprop="interactionStatistic" itemtype="https://schema.org/InteractionCounter" itemscope>
				<meta itemprop="userInteractionCount" content="' . ($__templater->method($__vars['post'], 'isContentVotingSupported', array()) ? $__templater->escape($__vars['post']['vote_score']) : $__templater->escape($__vars['post']['reaction_score'])) . '" />
	...
	if ($__templater->method($__vars['post'], 'canReport', array())) {
		$__compilerTemp2 .= '
				<a href="' . $__templater->func('link', array('posts/report', $__vars['post'], ), true) . '"
					...
					data-xf-click="overlay" data-cache="false">' . 'Report' . '</a>
			';
	}
	$__compilerTemp2 .= '
