 ...
 * @property bool $avatar_optimized
 ...
 * @property-read string $email_confirm_key
 * @property-read int $warning_count
 * @property-read int $next_allowed_username_change
 *
 * RELATIONS
 ...
 * @property-read UserBan|null $Ban
...
{
	public const GROUP_GUEST = 1;
	public const GROUP_REG = 2;
	public const GROUP_ADMIN = 3;
	public const GROUP_MOD = 4;
	...
	public function getPermissionCombinationId()
	{
		return $this->user_state == 'valid'
			...
			: UserRepository::$guestPermissionCombinationId;
	}
	...
	public function getClientSideCacheKey(): string
	{
		...
		$cacheKey = 's' . $style->getId()
			. ($style->isVariationsEnabled() && $this->style_variation)
				  ? '_v' . $this->style_variation
				  : ''
			. '_l' . $language->getId()
			. '_m' . $style->getLastModified()
			. '_u' . $this->user_id
			. '_p' . $period;
	...
	public function canStartConversationWith(User $user)
	{
		...
		if ($user->user_id == $this->user_id && $this->canStartConversation())
		{
			return true;
		}
	...
	public function canChangeEmail(&$error = null): bool
	{
		...
		$cutOff = \XF::$time - 3600;
		$changes = $this->repository(ChangeLogRepository::class)->countChangeLogsSince(
			'user',
			$this->user_id,
			'email',
			$cutOff
	...
	protected function verifyUsername(&$username)
	{
		...
		$validator = $this->app()->validator(Username::class);
	...
	protected function verifyEmail(&$email)
	{
		...
		if (!$emailValidator->isValid($email, $errorKey))
		{
			if ($errorKey == 'banned')
			{
				...
				$emailBan = $this->_em->findOne(BanEmail::class, ['banned_email' => $email]);
				if (!$emailBan)
				{
					...
					if ($bannedEmail)
					{
						$emailBan = $this->_em->findOne(BanEmail::class, ['banned_email' => $bannedEmail]);
					}
				}
				if ($emailBan)
				{
		...
		$existingUser = $this->finder(UserFinder::class)->where('email', $email)->fetchOne();
	...
	{
		if ($styleId && !$this->_em->find(\XF\Entity\Style::class, $styleId))
		{
			$styleId = 0;
		}
	...
	{
		if ($languageId && !$this->_em->find(Language::class, $languageId))
		{
			$languageId = 0;
		}
	...
	protected function _postSave()
	{
		...
		if ($this->isUpdate() && $this->isChanged('username') && $this->getExistingValue('username') != null)
		{
			...
			{
				$this->app()->jobManager()->enqueue(UserRenameCleanUp::class, [
		...
		if ($this->isChanged('is_staff'))
		{
			if ($this->isUpdate())
			{
				$this->repository(UserIgnoredRepository::class)->rebuildIgnoredCacheByIgnoredUser($this->user_id);
			}

			$this->repository(MemberStatRepository::class)->emptyCache('staff_members');
		}
		...
		{
			$this->repository(UserConfirmationRepository::class)->fastDeleteUserConfirmationRecords($this, 'security_lock_reset');
		}
	}
	...
	)
	{
		...
		if ($result->getResultType() === EntityResult::TYPE_WEBHOOK)
		{
			$result->includeColumn('email');
	...
	public static function getStructure(Structure $structure)
	{
		...
		$structure->columns = [
			...
			'style_variation' => ['type' => self::STR, 'default' => '', 'maxLength' => 50, 'changeLog' => false],
			...
			'avatar_optimized' => ['type' => self::BOOL, 'default' => false, 'changeLog' => false],
			'gravatar' => ['type' => self::STR, 'maxLength' => 120, 'default' => '',
				'match' => self::MATCH_EMAIL_EMPTY,
			],
