...
use function array_key_exists, count, floatval, in_array, intval, is_array, is_int, is_string, strlen, strval;
...
class Request
{
	...
	protected $isRocketLoaderDisabled = null;
	...
	protected static $jsonPhpInput = null;

	public function __construct(
		InputFilterer $filterer,
		?array $input = null,
		?array $files = null,
		?array $cookie = null,
		?array $server = null
	)
	{
		...
		if ($input === null)
		{
			...
			if (self::$jsonPhpInput === null)
			{
				self::$jsonPhpInput = $this->getPhpInputJson();
			}

			$input = self::$customMethodPhpInput + self::$jsonPhpInput + $_POST + $_GET;
		}
		if ($files === null)
		{
			$files = $_FILES;
	...
	public function getFile($key, $multiple = false, $skipErrors = true)
	{
		...
		foreach ($files AS $idx => $file)
		{
			...
			if ($file['name'] == 'blob' && preg_match('#^image/(pjpeg|jpeg|gif|png|webp)$#', $file['type'], $match))
			{
				switch ($match[1])
				{
					case 'jpeg':
			...
			$class = \XF::extendClass(Upload::class);
			/** @var Upload $upload */
			$upload = new $class($file['tmp_name'], $file['name'], $file['error']);

			if (!$multiple)
			{
				$exif = $this->getFileExifData();
				if ($exif)
				{
					$upload->setExif($exif);
				}
	 ...
	 */
	protected function getFileExifData(): array
	{
		...
		$tags = $this->filter('_xfExif', 'json-array');
		if (!$tags)
		{
			return [];
		}
		...
		$exifData = \XF::app()->data(Exif::class);
		return $exifData->getExifData($tags);
	}
	...
	public function isRocketLoaderDisabled(): bool
	{
		if ($this->isRocketLoaderDisabled === null)
		{
			$this->isRocketLoaderDisabled = \XF::config('disableRocketLoader')
				?? $this->ipMatchesRanges(
					$this->getServer('REMOTE_ADDR'),
					static::$cloudFlareIps
				);
		}

		return $this->isRocketLoaderDisabled;
	}

	protected function ipMatchesRanges($ip, array $ranges)
	{
		$ip = Ip::stringToBinary($ip, false);
		if ($ip === false)
		{
			return false;
		}
		...
		foreach ($ranges[$type] AS $range)
		{
			...
			$rangeIp = Ip::stringToBinary($range[0]);
			$cidr = intval($range[1]);
	...
	/**
	 * @deprecated This is deprecated as of XF 2.3 and will be removed in a
	 * future version.
	 */
	...
	public function getRobotName()
	{
		if ($this->robotName === null)
		{
			...
			if ($userAgent)
			{
				$this->robotName = \XF::app()->data(Robot::class)->userAgentMatchesRobot($userAgent);
			}
			else
			{
				$this->robotName = '';
	...
	{
		if (!preg_match('#^HTTP/([\d\.]+)$#', $this->getServer('SERVER_PROTOCOL'), $matches))
		{
			return null;
		}
	...
	{
		return trim($this->getServer('HTTP_AUTHORIZATION'));
	}
	...
	public function getXfRootPath()
	{
		...
		$trailingPath = File::stripRootPathPrefix($scriptPath);
		if (!$trailingPath)
		{
	...
	public function getRequestQueryParams(bool $stripRoute = false): array
	{
		...
		$queryParams = Arr::parseQueryString($queryString);
